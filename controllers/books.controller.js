...
const getBookById = async (req, res) => {
    try {
        ...
        if (book) {
            ...
            res.status(404).json({ status: Status.FAIL, message: 'Book not found', data: null });
        }
    } catch (err) {
...
const borrowBook = async (req, res) => {
    try {
        ...
        res.status(500).json({ status:Status.FAIL, message: 'Failed to borrow book from controller' });
    }
};
